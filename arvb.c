...
TAB *Cria(int t){
  ...
  novo->nchaves = 0;
  novo->chave =(TABNO**)malloc(sizeof(TABNO*)*((t*2)-1));
  novo->folha=1;
...
void Imprime(TAB *a, int andar){
  if(a){
    ...
    for(i=0; i<=a->nchaves-1; i++){
      ...
      printf("%d", a->chave[i]->chave); 
      visita_info(a->chave[i]->tipo,a->chave[i]->elem);
      printf("\n");
    }
    Imprime(a->filho[i],andar+1);
  }
...
TAB *Busca(TAB* x, TABNO *ch){
  ...
  if(!x) return resp;
  int i = 0;
  while(i < x->nchaves && ch->chave > x->chave[i]->chave) i++;
  if(i < x->nchaves && ch->chave == x->chave[i]->chave) return x;
...
TAB *Insere_Nao_Completo(TAB *x, TABNO *k, int t){
  int i = x->nchaves-1;
  if(x->folha){
    while((i>=0) && (k->chave<x->chave[i]->chave)){
  ...
  while((i>=0) && (k->chave<x->chave[i]->chave)) i--;
  i++;
  if(x->filho[i]->nchaves == ((2*t)-1)){
    x = Divisao(x, (i+1), x->filho[i], t);
    if(k->chave>x->chave[i]->chave) i++;
  }
...
TAB *Insere(TAB *T, TABNO *k, int t){
